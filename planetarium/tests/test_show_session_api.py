...
from planetarium.serializers import (AstronomyShowListSerializer,
                                     AstronomyShowDetailSerializer, ShowSessionListSerializer,
                                     ShowSessionDetailSerializer)
from planetarium.tests.model_samples import sample_astronomy_show, sample_show_theme, sample_show_session, \
...
class AuthenticatedAstronomyShowTests(TestCase):
    ...
    def test_show_session_list_filter_by_date(self):
        show_time1 = (timezone.now().astimezone(
            timezone.get_current_timezone()
        ) + timezone.timedelta(days=10)).isoformat()
        show_time2 = (timezone.now().astimezone(
            timezone.get_current_timezone()
        ) + timezone.timedelta(days=9)).isoformat()
        show_time3 = (timezone.now().astimezone(
            timezone.get_current_timezone()
    ...
    def test_show_session_list_filter_by_astronomy_show(self):
        ...
        res = self.client.get(SHOW_SESSION_URL, {
            "astronomy_show": f"{astronomy_show1}"}
                              )
